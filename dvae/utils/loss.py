...
def loss_KLD(z_mean, z_logvar, z_mean_p=0, z_logvar_p=0):
	if torch.div(z_logvar.exp() + (z_mean - z_mean_p + 1e-8).pow(2), z_logvar_p.exp()+1e-8).isnan().any():
		print(torch.div(z_logvar.exp() + (z_mean - z_mean_p + 1e-8).pow(2), z_logvar_p.exp()+1e-8))
